...
PARAM_STRING("input_model_file", "File to load LSH model from.  (Cannot be specified with --reference_file.)", "m", "");
...
PARAM_INT("projections", "The number of hash functions for each table", "K", 10);
PARAM_INT("tables", "The number of hash tables to be used.", "L", 30);
PARAM_DOUBLE("hash_width", "The hash width for the first-level hashing in the LSH preprocessing. By default, the LSH class automatically estimates a hash width for its use.", "H", 0.0);
PARAM_INT("second_hash_size", "The size of the second level hash table.", "S", 99901);
...
int main(int argc, char *argv[]) 
{
    ...
    if (!CLI::HasParam("input_model_file") && !CLI::HasParam("reference_file")) 
    {
        Log::Fatal << "Must specify either --input_model_file or --reference_file!" << endl;
    }

    if (!CLI::HasParam("neighbors_file") && !CLI::HasParam("distances_file") && !CLI::HasParam("output_model_file")) 
    {
        Log::Warn << "Neither --neighbors_file, --distances_file, nor --output_model_file are specified; no results will be saved." << endl;
    }

    if ((CLI::HasParam("query_file") && !CLI::HasParam("k")) ||  (!CLI::HasParam("query_file") && CLI::HasParam("k"))) 
    {
        Log::Fatal << "Both --query_file and --k must be specified if search is to be done!" << endl;
    }

    if (CLI::HasParam("input_model_file") && CLI::HasParam("k") && !CLI::HasParam("query_file")) 
    {
    ...
    if (CLI::HasParam("reference_file")) 
    {
        ...
        allkann.Train(referenceData, numProj, numTables, hashWidth, secondHashSize, bucketSize);
    ...
    if (CLI::HasParam("k")) 
    {
        Log::Info << "Computing " << k << " distance approximate nearest neighbors." << endl;
